...
class Involution(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.involution = Involution2d(c1, c2, stride=s, groups=g, bias=False)
...
class efficientnetBlock(nn.Module):
    def __init__(self, c1, c2, stage=None, model_name='tf_efficientnet_b2_ns'):
        super(efficientnetBlock, self).__init__()
        self.stage = stage 
        if stage == None:
            self.model = timm.create_model(model_name, features_only=True, pretrained=True, in_chans=3)

    def forward(self, x):
        if self.stage == None:
            x = self.model(x)
            return x 
        else: return x[self.stage]


class CSPResNeXtBlock(nn.Module):
    def __init__(self, c1, c2, stage=None, model_name='cspresnext50'):
...
class MobileViTBlock(nn.Module):
    def __init__(self, c1, c2, stage=None, model_name='mobilevit_s'):
