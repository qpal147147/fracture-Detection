...
def evaluate(
    ...
    print('\nmAP:{:.4f}'.format(np.mean([ap for ap, num in average_precisions.values()])))
    for label in range(generator.num_classes()):
        label_name = generator.label_to_name(label)
        print('{} AP: {:.4f}'.format(label_name, average_precisions[label][0]), end='  ')
        print("Precision: {:.4f}".format(precisions[label]), end='  ')
        print("Recall: {:.4f}".format(recalls[label]))
